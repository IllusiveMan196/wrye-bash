...
def _unicode_or_none(uni_str):
    return None if uni_str.lower() == u'none' else uni_str

def _int_or_none(uni_str):
    try:
        return int(uni_str)
    except ValueError:
        return None
...
class FidReplacer(_HandleAliases):
    ...
    def _parse_line(self, csv_fields):
        ...
        oldEid = _unicode_or_none(oldEid)
        newEid = _unicode_or_none(newEid)
        ...
        while len(_effects) >= 13:
            ...
            if range_:
                range_ = recipientTypeName_Number.get(range_.lower(),
                                                      _coerce(range_,int))
            actorvalue = _unicode_or_none(actorvalue)
            if actorvalue:
            ...
            seschool = _unicode_or_none(seschool)
            if seschool:
            ...
            if sevisuals is None:
                sevisuals = _unicode_or_none(sevisual)
            else:
            ...
            sename = _unicode_or_none(sename)
...
class ItemPrices(_HandleAliases):
    ...
    def _parse_line(self, csv_fields):
        ...
        value = _coerce(value, int)
        eid = _unicode_or_none(eid)
        itm_name = _unicode_or_none(itm_name)
