...
class Game(object):
    ...
    def _clean_actives(self, active, lord): # LO matters only for AsteriskGame
        ...
        if active_dropped:
            bolt.deprint(f'Removed '
                f'{u", ".join(u"%s" % s for s in sorted(active_dropped))} '
                f'from {self.get_acti_file()}')
    ...
    def _fix_load_order(self, lord, fix_lo):
        ...
        if master_dex > 0:
            bolt.deprint(f'{master_name} has index {master_dex:d} (must be 0)')
